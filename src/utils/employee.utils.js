class EmployeeUtils {
   ...
   */
  static #countScheduledDaysInRange(start, end, scheduledDays) {
   ...
   */
  static totalWorkDaysFromApprovedVacationRequests(vacationRequests, employeeWorkdays) {
    if (!Array.isArray(vacationRequests)) return 0;

    const scheduledDays =
      Array.isArray(employeeWorkdays) && employeeWorkdays.length > 0
        ? employeeWorkdays
            .map((w) => EmployeeUtils.DAY_NAME_TO_UTC[w.name])
    ...
    return vacationRequests
      ...
      .reduce(
        (sum, r) =>
          sum + EmployeeUtils.#countScheduledDaysInRange(r.start, r.end, scheduledDays),
        0,
      );
  }

  /** start/end format: 1970-01-01T08:00:00.000Z */
  static #countWorkdayHours(workday) {
    if (!workday.start || !workday.end) return 0;

    return new Date(workday.end).getHours() - new Date(workday.start).getHours();
  }

  static countWorkdayDays(workdays) {
    if (!Array.isArray(workdays)) return 0;

    return workdays.length;
  }

  static countWorkdaysHours(workdays) {
    if (!Array.isArray(workdays)) return 0;

    return workdays.reduce((prev, curr) => prev + EmployeeUtils.#countWorkdayHours(curr), 0);
  }
}
