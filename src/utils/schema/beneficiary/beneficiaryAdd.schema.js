...
const getAgeInYearsFromBirthDate = (birthDateStr) => {
...
export const beneficiaryCreateSchema = z
    .object({
        ...
        age_entered_house: z.coerce
            .number({
                ...
                invalid_type_error:
                    "La edad al entrar en la casa es obligatoria",
            })
    ...
    .superRefine((data, ctx) => {
        if (!DATE_REGEX.test(data.birth_date)) return;

        const birthDate = new Date(`${data.birth_date}T00:00:00`);
        if (Number.isNaN(birthDate.getTime())) return;

        const maxAge = getAgeInYearsFromBirthDate(data.birth_date);
        const ageEntered = data.age_entered_house;

        if (
            typeof ageEntered === "number" &&
            !Number.isNaN(ageEntered) &&
            ageEntered > maxAge
        ) {
            ctx.addIssue({
                code: z.ZodIssueCode.custom,
                message: `La edad al entrar en la casa no puede ser mayor que la edad actual del niño (${maxAge} años)`,
                path: ["age_entered_house"],
            });
        }
    });
