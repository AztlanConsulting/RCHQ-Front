        ...
        <div className="flex min-w-0 flex-1 flex-col gap-6 justify-between">
          <div className="h-full flex justify-between">
            <div className="w-full min-w-0">
              ...
              <Type variant="subtitle" as="p" className="mt-1">
                {employeeHouse?.name ? "Casa - " + String(employeeHouse.name) : ""}
              </Type>
            </div>
            <div>
              <button
          ...
          <div className="w-[97%] mr-auto flex flex-wrap justify-between">
            <div className="basis-1/4">
              <Type variant="metric-label" as="p">Puesto</Type>
              <Type variant="metric-value" as="p" className="mt-0.5">
                {employee?.role
                  ? String(employee.role).slice(0, 1).toUpperCase() + String(employee.role).slice(1)
                  : "N/A"}
              </Type>
            </div>
            <div className="basis-1/4">
              <Type variant="metric-label" as="p">Fecha de Nacimiento</Type>
              <Type variant="metric-value" as="p" className="mt-0.5">
                {employee?.birthDate ? String(employee.birthDate).slice(0, 10) : "N/A"}
              </Type>
            </div>
            <div className="basis-1/4">
              <Type variant="metric-label" as="p">Fecha de Inicio</Type>
              <Type variant="metric-value" as="p" className="mt-0.5">
                {employee?.startDate ? String(employee.startDate).slice(0, 10) : "Sin fecha de Inicio"}
              </Type>
            </div>
            <div className="basis-1/4">
              <Type variant="metric-label" as="p">Fecha de Terminación</Type>
              <Type variant="metric-value" as="p" className="mt-0.5">
          ...
          <Drawer isOpen={infoDrawer.isOpen} className={infoDrawer.isOpen ? "mt-4" : ""}>
            <div className="pt-4 border-t border-slate-200">
              <div className="w-[97%] mr-auto flex flex-wrap justify-between">
                <div className="basis-1/4">
                  <Type variant="metric-label" as="p">CURP</Type>
                  <Type variant="metric-value" as="p" className="mt-0.5">
                    {employee?.curp ?? "N/A"}
                  </Type>
                </div>
                <div className="basis-1/4">
                  <Type variant="metric-label" as="p">NSS</Type>
                  <Type variant="metric-value" as="p" className="mt-0.5">
                    {employee?.nss ?? "N/A"}
                  </Type>
                </div>
                <div className="basis-1/4">
                  <Type variant="metric-label" as="p">RFC</Type>
                  <Type variant="metric-value" as="p" className="mt-0.5">
                    {employee?.rfc ?? "N/A"}
                  </Type>
                </div>
                <div className="basis-1/4">
                  <Type variant="metric-label" as="p">Cuenta Bancaria</Type>
                  <Type variant="metric-value" as="p" className="mt-0.5">
          ...
          <div className="basis-1/3 p-2 rounded-xl border border-slate-200 bg-white p-8 shadow-sm">
            ...
            <div className="mt-4 flex flex-col gap-4">
              <div className="min-w-[12rem] flex-1">
                <Type variant="metric-label" as="p" className="mb-1.5">Correo Electronico</Type>
                <div className="flex items-center rounded-lg bg-neutral-50 px-4 py-2 shadow-[inset_0px_4px_4px_#00000040]">
                  <Type variant="metric-value" as="p">{employee?.email ?? "N/A"}</Type>
                </div>
              </div>
              <div className="min-w-[12rem] flex-1">
                <Type variant="metric-label" as="p" className="mb-1.5">Número de Telefono</Type>
                <div className="flex items-center rounded-lg bg-neutral-50 px-4 py-3 shadow-[inset_0px_4px_4px_#00000040]">
                  <Type variant="metric-value" as="p">{employee?.phoneNumber ?? "N/A"}</Type>
                </div>
              </div>
              <div className="min-w-[12rem] flex-1">
                <Type variant="metric-label" as="p" className="mb-1.5">Dirección</Type>
                <div className="flex items-center rounded-lg bg-neutral-50 px-4 py-3 shadow-[inset_0px_4px_4px_#00000040]">
                  <Type variant="metric-value" as="p">{employeeAddress?.street ?? "N/A"}</Type>
                </div>
              </div>
              <div className="min-w-[12rem] flex-1">
                <Type variant="metric-label" as="p" className="mb-1.5">Código Postal</Type>
                <div className="flex items-center rounded-lg bg-neutral-50 px-4 py-3 shadow-[inset_0px_4px_4px_#00000040]">
                  <Type variant="metric-value" as="p">{employeeAddress?.postalCode ?? "N/A"}</Type>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="mt-4 w-full flex flex-col gap-4">
              <div className="w-full flex justify-between">
                <div>
                  ...
                  <Type variant="metric-value" as="p" className="mt-0.5">
                    {employee?.type
                      ? String(employee.type).slice(0, 1).toUpperCase() + String(employee.type).slice(1)
                      : "N/A"}
                  </Type>
                </div>
                ...
                <ul>
                  {employeeWorkdays?.length > 0 &&
                    employeeWorkdays.map((workday) => (
                      <div key={workday.name} className="w-full flex justify-between">
                        <Type variant="metric-label">{workday.name}</Type>
