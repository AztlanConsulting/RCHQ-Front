...
describe("employeeUpdateService", () => {
    ...
    describe("updateBasicInfoService", () => {
        ...
        it("lanza error si la respuesta no es ok (400)", async () => {
            ...
            await expect(
                updateBasicInfoService(EMP_ID, body),
            ).rejects.toThrow();
        });
    ...
    describe("updateContactInfoService", () => {
        ...
        it("lanza error si no hay token", async () => {
            getToken.mockReturnValue(null);
            await expect(
                updateContactInfoService(EMP_ID, body),
    ...
    describe("updateAdminInfoService", () => {
        ...
        it("lanza error si no ok", async () => {
            ...
            await expect(
                updateAdminInfoService(EMP_ID, body),
            ).rejects.toThrow();
        });
    });
});
...
describe("EmployeeContactCard", () => {
    ...
    describe("Modo lectura (isEditing=false)", () => {
        ...
        it("muestra el botón de editar (lápiz)", () => {
            render(<EmployeeContactCard {...defaultProps} />);
            expect(
                screen.getByLabelText("Editar contacto"),
            ).toBeInTheDocument();
        });
        ...
        it("muestra 'N/A' si el email es null", () => {
            render(
                <EmployeeContactCard
                    {...defaultProps}
                    employee={{ ...mockEmployee, email: null }}
                />,
            );
        ...
        it("muestra 'N/A' si phoneNumber es undefined", () => {
            render(
                <EmployeeContactCard
                    {...defaultProps}
                    employee={{ ...mockEmployee, phoneNumber: undefined }}
                />,
            );
            expect(screen.getAllByText("N/A").length).toBeGreaterThan(0);
        });
        ...
        it("no muestra inputs de edición en modo lectura", () => {
            render(<EmployeeContactCard {...defaultProps} />);
            expect(
                screen.queryByRole("textbox", { name: /Correo/i }),
            ).toBeNull();
        });
    ...
    describe("Modo edición (isEditing=true)", () => {
        ...
        it("muestra los inputs de edición", () => {
            render(<EmployeeContactCard {...editingProps} />);
            expect(
                screen.getByDisplayValue("juan@mail.com"),
            ).toBeInTheDocument();
        ...
        it("llama a setContactField al cambiar el email", () => {
            ...
            expect(editingProps.setContactField).toHaveBeenCalledWith(
                "email",
                "nuevo@mail.com",
            );
        });
        ...
        it("deshabilita ambos botones cuando saving=true", () => {
            ...
            expect(
                screen.getByText("Guardar").closest("button"),
            ).toBeDisabled();
            expect(
                screen.getByText("Cancelar").closest("button"),
            ).toBeDisabled();
        });
...
describe("EmployeeBasicCard", () => {
    ...
    describe("Modo lectura", () => {
        ...
        it("muestra el botón lápiz", () => {
            render(<EmployeeBasicCard {...defaultProps} />);
            expect(
                screen.getByLabelText("Editar información básica"),
            ).toBeInTheDocument();
        });
        ...
        it("muestra el drawer toggle cuando no está editando", () => {
            render(<EmployeeBasicCard {...defaultProps} />);
            expect(
                screen.getByLabelText("Ver más información"),
            ).toBeInTheDocument();
        });
    ...
    describe("Modo edición", () => {
        ...
        it("llama a setBasicField al cambiar el nombre", () => {
            ...
            expect(editingProps.setBasicField).toHaveBeenCalledWith(
                "name",
                "Nuevo Nombre",
            );
        });
...
describe("EmployeeAdminCard", () => {
    ...
    describe("Modo lectura", () => {
        it("muestra el título", () => {
            ...
            expect(
                screen.getByText("Información Administrativa"),
            ).toBeInTheDocument();
        });
        ...
        it("muestra N/A cuando el salario es null", () => {
            render(
                <EmployeeAdminCard
                    {...defaultProps}
                    employee={{ ...mockEmployee, salary: null }}
                />,
            );
            expect(screen.getAllByText("N/A").length).toBeGreaterThan(0);
        });
    ...
    describe("Modo edición", () => {
        ...
        it("llama a setWorkdayTime al cambiar la hora de inicio", () => {
            ...
            expect(editingProps.setWorkdayTime).toHaveBeenCalledWith(
                "wd1",
                "start",
                "09:00",
            );
        ...
        it("llama a setAdminField al cambiar el salario", () => {
            ...
            expect(editingProps.setAdminField).toHaveBeenCalledWith(
                "salary",
                "20000",
            );
        });
