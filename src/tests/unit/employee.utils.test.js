...
describe("formatMexicoLongWeekdayCalendarDate", () => {
  it("retorna — para valor inválido", () => {
    expect(Dates.formatMexicoLongWeekdayCalendarDate(null)).toBe("—");
    expect(Dates.formatMexicoLongWeekdayCalendarDate("")).toBe("—");
  });

  it("formatea YYYY-MM-DD con domingo 3 may 2026 (+6h, UTC)", () => {
    const s = Dates.formatMexicoLongWeekdayCalendarDate("2026-05-03");
...
describe("formatMexicoDayMonthCommaTime12h", () => {
  it("retorna — para valor inválido", () => {
    expect(Dates.formatMexicoDayMonthCommaTime12h(null)).toBe("—");
  });

  it("incluye día, mes y hora en México (sin +6h sobre ISO; 10:00Z → 4:00am CDMX)", () => {
    const s = Dates.formatMexicoDayMonthCommaTime12h("2026-05-01T10:00:00.000Z");
...
describe("coerceEventDateInput", () => {
  it("acepta YYYY-MM-DD", () => {
    const d = Dates.coerceEventDateInput("2026-05-05");
