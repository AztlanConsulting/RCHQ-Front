...
import { createGlobalEvent, getEventTypes } from "../../services/eventService";
...
const waitForGlobalForm = async () => {
    await screen.findByRole("option", { name: /festivo/i });
};

const getEventTypeSelect = () => screen.getAllByRole("combobox")[0];

const fillRequiredFields = async () => {
    fireEvent.change(screen.getByPlaceholderText("Agregar título"), {
        target: { value: "Día festivo nacional" },
    });
    ...
    fireEvent.change(screen.getByPlaceholderText("Agregar descripción ..."), {
        target: { value: "Festivo oficial para todas las casas." },
    });
};
...
describe("Integración: agregar evento global", () => {
    beforeEach(() => {
        vi.clearAllMocks();
        localStorage.clear();
        ...
        createGlobalEvent.mockResolvedValue({
            success: true,
            data: {
                globalEvent: {
                    globalEventId: "evt-global-1",
    ...
    it("obtiene y muestra los tipos de evento al abrir el modal", async () => {
        ...
        expect(
            screen.getByRole("option", { name: /institucional/i }),
        ).toBeInTheDocument();
    });
    ...
    it("crea un evento global con los datos del formulario", async () => {
        const { onClose, onSuccess } = await renderModal();
        await waitForGlobalForm();
        ...
        await waitFor(() => {
            expect(createGlobalEvent).toHaveBeenCalledTimes(1);
        });
    ...
    it("muestra el modal de empalme cuando el backend regresa colisiones", async () => {
        createGlobalEvent.mockResolvedValueOnce({
            success: false,
            data: {
                collisions: [
                    {
                        globalEventId: "collision-1",
    ...
    it("permite forzar el registro cuando hay empalme", async () => {
        ...
        await waitFor(() => {
            expect(createGlobalEvent).toHaveBeenCalledTimes(2);
        });

        expect(createGlobalEvent).toHaveBeenLastCalledWith(
            expect.objectContaining({
                forceOverlap: true,
            }),
        );
    ...
    it("muestra error del servidor si falla createGlobalEvent", async () => {
        createGlobalEvent.mockRejectedValueOnce(
            new Error("Error al registrar el evento global"),
        );
        ...
        expect(await screen.findByRole("alert")).toHaveTextContent(
            "Error al registrar el evento global",
        );
    });
});
