...
vi.mock("../../services/documentService", () => ({
    getDocumentsService: vi
        .fn()
        .mockResolvedValue({ success: true, body: null }),
...
beforeEach(() => {
    ...
    updateBasicInfoService.mockResolvedValue({
        success: true,
        message: "Información básica actualizada con éxito",
    });
    updateContactInfoService.mockResolvedValue({
        success: true,
        message: "Información de contacto actualizada con éxito",
    });
    updateAdminInfoService.mockResolvedValue({
        success: true,
...
describe("DetalleEmpleado — editar información básica", () => {
    it("abre el formulario de edición al hacer click en el lápiz de básica", async () => {
        renderPage();
        fireEvent.click(
            await screen.findByLabelText("Editar información básica"),
        );
        await waitFor(() => {
...
describe("DetalleEmpleado — editar información de contacto", () => {
    it("abre el formulario de edición de contacto", async () => {
        ...
        await waitFor(() => {
            expect(
                screen.getByDisplayValue("carlos@mail.com"),
            ).toBeInTheDocument();
        });
    });
...
describe("DetalleEmpleado — editar información administrativa", () => {
    it("abre el formulario de edición administrativa", async () => {
        renderPage();
        fireEvent.click(
            await screen.findByLabelText("Editar información administrativa"),
        );
        await waitFor(() => {
            expect(screen.getByText("Guardar")).toBeInTheDocument();
            expect(screen.getByText("Cancelar")).toBeInTheDocument();
    ...
    it("muestra las opciones de roles después de cargar catálogos", async () => {
        ...
        await waitFor(() => {
            const adminOption = screen.getAllByRole("option", {
                name: "Admin",
                hidden: true,
            });
            expect(adminOption.length).toBeGreaterThan(0);

            const coordinadorOption = screen.getByRole("option", {
                name: "Coordinador",
                hidden: true,
            });
