...
describe("Integración: trabajador consulta sus ausencias", () => {
    ...
    it("abre el detalle de una ausencia propia con evidencia sin mostrar controles administrativos", async () => {
        ...
        expect(
            within(dialog).getByText("Tipo de ausencia:"),
        ).toBeInTheDocument();
        expect(within(dialog).getByText("Médica")).toBeInTheDocument();
        expect(
            within(dialog).getByText("Reposo indicado por médico"),
        ).toBeInTheDocument();
        expect(
            within(dialog).queryByText("Nombre del trabajador"),
        ...
        fireEvent.click(
            within(dialog).getByRole("button", { name: /ver evidencia/i }),
        );
    ...
    it("no se rompe con ausencia sin evidencia, campos vacíos y fechas raras", async () => {
        ...
        expect(
            within(dialog).queryByRole("button", { name: /ver evidencia/i }),
        ).not.toBeInTheDocument();
        expect(
            within(dialog).queryByRole("button", { name: /editar/i }),
        ).not.toBeInTheDocument();
    ...
    it("filtra por evidencia y no convierte URLs de eventos globales en evidencia", async () => {
        ...
        await waitFor(() =>
            expect(
                screen.getByText("Evento global con URL"),
            ).toBeInTheDocument(),
        );
        ...
        expect(
            within(dialog).getByText("Detalle del evento"),
        ).toBeInTheDocument();
        expect(
            within(dialog).queryByRole("button", { name: /ver evidencia/i }),
        ).not.toBeInTheDocument();
        ...
        fireEvent.click(
            screen.getByRole("checkbox", { name: /con evidencia/i }),
        );

        await waitFor(() =>
            expect(
                screen.queryByTestId("calendar-event-absence-evidence"),
            ).not.toBeInTheDocument(),
        );
        expect(
            screen.getByTestId("calendar-event-absence-no-evidence"),
        ).toBeInTheDocument();
