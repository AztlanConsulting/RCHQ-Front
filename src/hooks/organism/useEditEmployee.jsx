...
export const useEditEmployee = (employeeId, onSuccess) => {
    ...
    const openBasicEdit = useCallback((employee) => {
        ...
        setBasicFormState({
            ...
            nss: employee?.nss ?? "",
            bankAccount:
                employee?.bankAccount && !isNaN(employee.bankAccount)
    ...
    const setBasicField = useCallback((field, value) => {
        ...
        if (field === "name" || field === "surname")
            finalValue = finalValue.slice(0, 50);
    ...
    const submitBasic = useCallback(async () => {
        ...
        try {
            ...
            if (!validation.success) {
                ...
                throw new Error(
                    firstIssue?.message ||
                        "Por favor, llena todos los campos obligatorios correctamente.",
                );
            }
    ...
    const submitContact = useCallback(async () => {
        ...
        try {
            const validation =
                employeeContactUpdateSchema.safeParse(contactForm);
            if (!validation.success) {
                const firstIssue =
                    validation.error?.issues?.[0] ||
                    validation.error?.errors?.[0];
                throw new Error(
                    firstIssue?.message ||
                        "Es necesario completar todos los campos de contacto.",
                );
            }
    ...
    const submitAdmin = useCallback(async () => {
        ...
        try {
            if (
                ...
                throw new Error(
                    "Debes llenar todos los campos administrativos (Casa, Puesto, Tipo y Salario).",
                );
            }
            ...
            if (adminForm.type !== "Voluntariado" && salaryNum === 0) {
                throw new Error(
                    "El salario debe ser mayor a 0 para este tipo de contrato.",
                );
            }
            ...
            console.log(
                "frequencyOfPaymentId:",
                adminForm.frequencyOfPaymentId,
            );

            const selectedWorkdays = adminForm.selectedWorkdays.filter(
                (w) => w.selected,
            );
            ...
            const workdaysToSend = selectedWorkdays.map(
                ({ workdayId, name, start, end }) => {
                    if (!start || !end) {
                        throw new Error(
                            `Debes asignar un horario completo para el día ${name}.`,
                        );
                    }
                    ...
                    if (durationHours < 1) {
                        throw new Error(
                            `El turno del ${name} debe durar al menos 1 hora.`,
                        );
                    }
                    if (durationHours > 24) {
                        throw new Error(
                            `El turno del ${name} no puede durar más de 24 horas.`,
                        );
                    }
            ...
            if (!validation.success) {
                ...
                throw new Error(
                    firstIssue?.message || "Revisa los campos administrativos.",
                );
            }
