...
import {
    globalEventSchema,
    buildPayload,
} from "../../utils/schema/evento/globalEvent.schema";
...
export const useGlobalForm = ({
    ...
    const setField = useCallback((field, value) => {
        setForm((prev) => {
            const next = shiftDateTimeRange(prev, field, value);
            if (field === "isRecurring" && !value) {
                return { ...next, recurrenceType: "" };
            }
            return next;
        });
        ...
        setErrors((prev) => {
            ...
            if (field === "isRecurring" && !value) {
                return { ...next, recurrenceType: undefined };
            }
            return next;
        });
    }, []);
    ...
    const validate = () => {
        const input = {
            ...
            categoryKey: "global",
            forceOverlap: false,
            recurrenceType: form.isRecurring ? (form.recurrenceType || null) : null,
        };

        const result = globalEventSchema.safeParse(input);
    ...
    const submitPayload = async (payload) => {
        ...
        try {
            const response = await createGlobalEvent(payload);
    ...
    const handleForceOverlap = async () => {
        ...
        try {
            const response = await createGlobalEvent({
